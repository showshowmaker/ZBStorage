...
public:
    ...
    // 插入时使用
    int openFile_insert(const string& filePath, int flags) {
...
string logPath = "/mnt/NVMe/log2";//"../log2"
...
uint64_t allocateDiskSpace(char* buffer,int size) {
    ...
    // int fd = pool.openFile_insert(fileName, O_WRONLY | O_CREAT);
    if (fd == -1) {
...
void readFileInfoFromMap(const string& filePath, MasstreeWrapper& mt_wrapper, uint32_t uid,uint32_t lastuid)
{
    ...
    while (getline(inputFile, encryptedPath)&&num--) { //读单独拿出来 
        ...
        //string key = "user" + to_string(uid) + encryptedPath;
        // 填充随机数据
        ...
        for(uint32_t i=uid;i<=lastuid;i++)
            {
                ...
                // string key = "user" + to_string(i) + encryptedPath;
                // 读取文件信息
                readAndUpdateLeafNode(mt_wrapper, key, value);
            }
    }
...
int main() {
    ...
    for(int i=1;i<=1000;i++)
    {
        for(int j=0;j<1000;j++)
        {
            ...
            string key = generateID(allFilePaths[j],i);
            // gettimeofday(&t4,NULL);
            // hash_timeuse += t4.tv_sec - t3.tv_sec + (t4.tv_usec - t3.tv_usec)/1000000.0;
            // sub.tv_sec += t4.tv_sec - t3.tv_sec;
        }
    }
    gettimeofday(&t2,NULL);
    timeuse = t2.tv_sec - t1.tv_sec + (t2.tv_usec - t1.tv_usec)/1000000.0;
    cout << "total time:" << timeuse << "s" << endl;
    cout << "average find time:" << timeuse/(k*(100-1+1)) << "s" << endl;
    ...
    while(choose!=-1)
    {
        switch(choose)
        {
            ...
            case 5:
                ...
                // hash_timeuse = sub.tv_sec + sub.tv_usec/1000000.0;
                // cout << "hash_time:" << hash_timeuse << "s" << endl;
                break;
            case 6:
                // 删除 log 目录下所有文件
                deleteAllFilesInDirectory(logPath);    
