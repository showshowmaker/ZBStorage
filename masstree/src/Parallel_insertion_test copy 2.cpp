...
string logPath = "../log";//"../log" //"/mnt/NVMe/log"
...
bool readfromLeafNode(MasstreeWrapper& masstree_wrapper, const string& key) {
    ...
    if (result.empty()) {
        ...
        while (low <= high) {
                mid = low + (high - low) / 2;
                //cout<<"mid:  "<<mid+1<<endl;
                //memcpy(keyy, ptr + mid * (keysize+valuesize), keysize);
                //cout<<"此时访问uid："<<(int)keyy[0]<<endl;
        ...
        cout << "Value not found for key: " << key << endl;
                cout<<"fileIndex:  "<<oldnum/ (filesize * 1024 / pagesize)<<endl;
        cout<<"size:  "<<size<<endl;
        char otherProperties[valuesize-38];
        char keyy[keysize];
        for(int i=0;i<size;i++)
        {
            memcpy(keyy, buffer + i * (keysize+valuesize), keysize);
            memcpy(otherProperties, buffer + i * (keysize+valuesize)+keysize+38, valuesize-38);
            cout<<"第"<<i+1<<"   "<<(int)keyy[0]<<"    "<<otherProperties<<endl;
        }
        return false;
    }
}
...
void readvectorfromtxt(const string& filePath)
{
    ...
    for(int i=0;i<userfilenum;i++)
    {
        ...
        memcpy(fileInfo.otherProperties,encryptedPath.c_str(),valuesize-38);
        //fillRandomData(fileInfo.otherProperties, valuesize-40);
        getline(inputFile, encryptedPath);
        inputFile >> fileInfo.type >> fileInfo.size >> fileInfo.createTime >> fileInfo.lastModifiedTime >> fileInfo.lastAccessTime;
        fillRandomData(fileInfo.storageLocation, 5);
...
void insertfromvector(MasstreeWrapper& mt_wrapper,uint32_t firstuid,uint32_t lastuid)
{
    ...
    for(uint32_t uid=firstuid;uid<=lastuid;uid++)
    {
        for(int i=0;i<userfilenum;i+=classsize)
        {
            ...
            mtx.lock();
            cout<<"user:"<<uid<<" file:"<<nownumEntries<<endl;
            mtx.unlock();
            ostringstream oss;
            ...
            for (int j = i; j <last; j++)
            {
                ...
                if((j+1)%leafnum==0)
                {
                    address=nownumEntries*(filesize * 1024 / pagesize)+(j-i)/leafnum;
                    mt_wrapper.insert(key,(address<<8)|leafnum);
                }else if(j==last-1)
                {
                    address=nownumEntries*(filesize * 1024 / pagesize)+(j-i)/leafnum;
                    mt_wrapper.insert(key,(address<<8)|(j-i+1)%leafnum);
                }
                oss<<key;
                ...
                oss.write(fileInfos[j].storageLocation, 5);
                oss.write(fileInfos[j].otherProperties, valuesize-38);
            }
            // 写入文件
...
int main() {
    ...
    allFilePaths.clear();
                ...
                for (int j=0;j<10;j++) { //加随机数量 从0.1B往上加
                        //随机生成n
                        n = rand()%userfilenum;
                        //忽略文件前n行
                        for(int m=0;m<n;m++)
...
//创建输出文件流对象并打开文件
    ...
    {
        for(int usrsPerTime=1;usrsPerTime<=2;usrsPerTime*=2)
        {
            int low=1;
            int high=10;
            MasstreeWrapper mt_wrapper;
            mt_wrapper.table_init();
...
for(int i=low;i<=high;i++)
{
    for(int j=0;j<10;j++)
    {
        ...
        key[0]=i;
        //查询文件信息
        cout<<i<<"  "<<fileInfos[j].otherProperties<<endl;
