...
string logPath = "/mnt/md0/logp";//"../log" //"/mnt/NVMe/log"
...
class LeafNodeLockManager {
private:
    std::unordered_map<uint64_t, std::mutex> locks;
    std::mutex mutex_; // 用于保护内部状态的互斥锁

public:
    std::mutex& getLock(uint64_t leafNodeID) {
        std::lock_guard<std::mutex> lock(mutex_); // 对内部状态加锁
        return locks[leafNodeID];
    }

    void removeLock(uint64_t leafNodeID) {
        std::lock_guard<std::mutex> lock(mutex_); // 对内部状态加锁
        locks.erase(leafNodeID);
    }
};
LeafNodeLockManager lockManager;
// 分配磁盘空间函数
...
uint64_t allocateDiskSpace(MasstreeWrapper& masstree_wrapper,char* buffer,int size) {
    uint64_t nownumEntries = numEntries.fetch_add(1);
    //cout<<"分配新地址:   "<<nownumEntries<<endl;
    ...
    // 获取叶子节点的互斥锁
    std::mutex& leafLock = lockManager.getLock(nownumEntries);
    // 锁定互斥锁
    leafLock.lock();
    masstree_wrapper.insert(string(buffer+(size-1)*(keysize+valuesize),keysize),(nownumEntries<<8)|size);
    ssize_t bytesWritten = pwrite(fd, buffer, (keysize+valuesize)*size, offset);
    if (bytesWritten == -1) {
        cerr << "Error writing file: " << fileName << endl;
            leafLock.unlock();
            //lockManager.removeLock(nownumEntries);
        return -1;
    }
    //masstree_wrapper.insert(string(buffer+(size-1)*(keysize+valuesize),keysize),(nownumEntries<<8)|size); 
        // 解锁互斥锁
...
void readAndUpdateLeafNode(MasstreeWrapper& masstree_wrapper, const string& key, char* value) {
    // //获取线程id
    // uint32_t uid = std::hash<std::thread::id>{}(std::this_thread::get_id());
    // masstree_wrapper.thread_init(uid);
    // 创建一个向量来存储扫描结果
    vector<pair<string,uint64_t>> result;
    masstree_wrapper.scan(key,1,result);
    if(result.empty())
    {
        ...
        {
            uint64_t newnum=allocateDiskSpace(masstree_wrapper,kv,1);
        }else
        {
            cout<<"插入小的"<<endl;
            uint64_t combinedData = result[0].second;
            ...
            //setWriteLock(fd, alignedOffset, pagesize*1024*1024);
            ...
            //unlock(fd, alignedOffset, pagesize*1024*1024);
        ...
        uint64_t combinedData=result[0].second;
        uint64_t oldnum=combinedData>>8;
        //std::mutex& leafLock = leafNodeLocks[oldnum];
        while (true) {
            //(leafNodeLocks[oldnum]).lock();
                // 获取叶子节点的互斥锁
    std::mutex& leafLock = lockManager.getLock(oldnum);
    // 锁定互斥锁
    leafLock.lock();
            // 在锁内部重新查找叶子节点
            result.clear();
            ...
            if(result.empty())
            {
                cout<<"新获取为空"<<endl;
            leafLock.unlock();
            //lockManager.removeLock(oldnum);
                return;
            }
            combinedData = result[0].second;
            if (oldnum == (combinedData >> 8)) {
                break; 
            }
            //(leafNodeLocks[oldnum]).unlock();
            leafLock.unlock();
            //lockManager.removeLock(oldnum);
            // 如果叶子节点发生了变化，重新执行循环获取锁和数据
            oldnum = combinedData >> 8;
        }
        std::mutex& leafLock = lockManager.getLock(oldnum);
        int size=combinedData&0xFF;
        // 计算文件索引和偏移量
        ...
        if(size==leafnum)
        {
            ...
            //lockManager.removeLock(oldnum);
                ...
                ptr+=keysize;
                memcpy(ptr, value, valuesize);
                uint64_t newnum=allocateDiskSpace(masstree_wrapper,buffer+(keysize+valuesize),leafnum/2+1);
                leafLock.unlock();
            //lockManager.removeLock(oldnum);
                return;
            }else
            {
                uint8_t leafNodeCount = leafnum-leafnum/2+1; 
                uint64_t combinedData = (oldnum << 8) | leafNodeCount;
                masstree_wrapper.insert(result[0].first,combinedData);
                uint64_t newnum=allocateDiskSpace(masstree_wrapper,buffer+(keysize+valuesize),leafnum/2);
...
void readvectorfromtxt(const string& filePath)
{
    ...
    FileInfo fileInfo;
    int num=userfilenum;
    while (getline(inputFile, fileInfo.encryptedPath)&&num--) { //读单独拿出来 
        inputFile >> fileInfo.nameLength >> fileInfo.type >> fileInfo.size >> fileInfo.createTime >> fileInfo.lastModifiedTime >> fileInfo.lastAccessTime;
        inputFile.ignore(); // 忽略换行符
        fileInfos.push_back(fileInfo);
    }
    inputFile.close();
}
void insertfromvector(MasstreeWrapper& mt_wrapper,uint32_t start,uint32_t end,uint32_t uid,uint32_t lastuid)
{
    mt_wrapper.thread_init(start);
    ...
    {
        offset = 0;
        memcpy(value + offset, &fileInfos[i].type, sizeof(char));
        offset += sizeof(char);
        memcpy(value + offset, &fileInfos[i].size, sizeof(uint64_t));
        offset += sizeof(uint64_t);
        memcpy(value + offset, &fileInfos[i].createTime, sizeof(time_t));
        offset += sizeof(time_t);
        memcpy(value + offset, &fileInfos[i].lastModifiedTime, sizeof(time_t));
        offset += sizeof(time_t);
        memcpy(value + offset, &fileInfos[i].lastAccessTime, sizeof(time_t));
        offset += sizeof(time_t);
        ...
        key = generateID(fileInfos[i].encryptedPath,uid);
        readAndUpdateLeafNode(mt_wrapper, key, value);
        char* ptr;
        for(uint32_t j=uid+1;j<=lastuid;j++)
        {
            //cout<<"user"<<j<<": "<<i<<endl;
            ptr = &key[0];
...
//         {
//             //cout<<"i: "<<i<<"  path: "<<encryptedPath<<endl;
//             // 生成ID
...
int main() {
    ...
    // 创建输出文件流对象并打开文件
    ofstream outputFile("../txt/output1000.pooltask3.txt");
    ...
    // for(numThreads=20;numThreads<=70;numThreads+=5)
    // {
//大粒度   nopool
        // MasstreeWrapper mt_wrapper;
        ...
        // high=100;
        // gettimeofday(&t1,NULL);
        // numTasks=userfilenum;
        // // 计算每个线程需要处理的任务数量
        // tasksPerThread = numTasks / (numThreads);
        // remainingTasks = numTasks % (numThreads);
        // startTaskIndex=0;
        ...
        //     threads.emplace_back([startTaskIndex, endTaskIndex, &mt_wrapper,low,high]() { // 在捕获列表中添加 mt_wrapper
        //     // 计算该线程负责的任务范围
        //         insertfromvector(mt_wrapper, startTaskIndex, endTaskIndex,low,high);         
        //     });  
        ...
        //                 if(!readfromLeafNode(mt_wrapper,generateID(fileInfos[j].encryptedPath,i)))
        //                 {
        //                     cout<<i<<"  "<<fileInfos[j].encryptedPath<<endl;
        //                 }
        //             }
        //         }
        // deleteAllFilesInDirectory(logPath); 
//大粒度 可调粒度 pool
        // for(int tasksize=10;tasksize<=10;tasksize*=10)
        // {
        //     MasstreeWrapper mt_wrapper;
        //     cout<<"numThreads:"<<numThreads<<"  tasksize:"<<tasksize<<endl;
        //     ThreadPool tpool(numThreads);
        //     tpool.init();
        //     low=1;
        //     high=10000;
        //     gettimeofday(&t1,NULL);
        //     startTaskIndex=0;
        //     for (int t = 0;t<=userfilenum-tasksize; t+=tasksize) {
        //         //cout<<t<<"~"<<t+tasksize-1<<endl;
        //         tpool.submit(insertfromvector, std::ref(mt_wrapper),t,t+tasksize-1,low,high);    
        //     }
        //     tpool.wait();
        //     gettimeofday(&t2,NULL);
        //     timeuse = t2.tv_sec - t1.tv_sec + (t2.tv_usec - t1.tv_usec)/1000000.0;
...
//小粒度 pool
        ...
        // low=1;
        // high=100;
        // // for (int t = 0; t < userfilenum; t+=10000) {
        // //     //cout<<t<<"~"<<t+9999<<endl;
        // //     tpool.submit(insertfromvector, std::ref(mt_wrapper),t,t+9999,low,high);   
        // // }
        ...
        // timeuse = t2.tv_sec - t1.tv_sec + (t2.tv_usec - t1.tv_usec)/1000000.0;
        // cout << "total time:" << timeuse << "s" << endl;
        // cout << "average find time:" << timeuse/(1000*(high-low+1)) << "s" << endl;
        // for(int i=low;i<=high;i++)
        // {
        ...
        // deleteAllFilesInDirectory(logPath);
//  按路径
        // if(userfilenum%numThreads!=0||numThreads==userfilenum) continue;
        // MasstreeWrapper mt_wrapper;
        // cout<<"numThreads:"<<numThreads<<endl;
        // threads.clear();
        // low=1;
