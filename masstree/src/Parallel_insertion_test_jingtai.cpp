...
// 分配磁盘空间函数
std::vector<std::mutex> leafNodeLocks(10000000);
...
uint64_t allocateDiskSpace(MasstreeWrapper& masstree_wrapper,char* buffer,int size) {
    ...
    //(leafNodeLocks[nownumEntries]).lock();  
    ...
    (leafNodeLocks[nownumEntries]).lock(); 
    //setWriteLock(fd, offset, pagesize*1024*1024);
    masstree_wrapper.insert(string(buffer+(size-1)*(keysize+valuesize),keysize),(nownumEntries<<8)|size);
    ...
    if (bytesWritten == -1) {
        ...
        (leafNodeLocks[nownumEntries]).unlock();
        return -1;
    }
    //masstree_wrapper.insert(string(buffer+(size-1)*(keysize+valuesize),keysize),(nownumEntries<<8)|size); 
    (leafNodeLocks[nownumEntries]).unlock();
    //unlock(fd, offset, pagesize*1024*1024);
    // 返回叶子结点号
