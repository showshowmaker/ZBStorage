...
bool ArchiveChunkMetaStore::Init(const std::string& dir_path,
                                 size_t max_chunks,
    ...
    max_chunks_ = std::max<size_t>(1, max_chunks);
...
void ArchiveChunkMetaStore::SetMaxChunks(size_t max_chunks) {
    std::lock_guard<std::mutex> lock(mu_);
    max_chunks_ = std::max<size_t>(1, max_chunks);
    EvictIfNeededLocked();
}

void ArchiveChunkMetaStore::TrackAccess(const std::string& disk_id,
                                        const std::string& chunk_id,
                                        uint64_t end_offset,
                                        bool is_write,
                                        uint64_t checksum,
                                        uint64_t now_ms) {
    if (disk_id.empty() || chunk_id.empty() || !inited_) {
        return;
    }

    std::lock_guard<std::mutex> lock(mu_);
    const std::string key = BuildKey(disk_id, chunk_id);
    auto it = metas_.find(key);
    if (it == metas_.end()) {
        EvictIfNeededLocked();
        ArchiveChunkMeta meta;
        meta.disk_id = disk_id;
        meta.chunk_id = chunk_id;
        it = metas_.emplace(key, std::move(meta)).first;
    }

    ArchiveChunkMeta& meta = it->second;
    ...
    ++meta.version;
    ArchiveChunkMeta persisted = meta;
    EvictIfNeededLocked();
...
void ArchiveChunkMetaStore::RemoveChunk(const std::string& disk_id, const std::string& chunk_id) {
    if (disk_id.empty() || chunk_id.empty() || !inited_) {
        return;
    }
    ...
    metas_.erase(it);
    if (AppendWalRecordLocked("D\t" + disk_id + "\t" + chunk_id)) {
        ++updates_since_snapshot_;
        (void)MaybeSnapshotLocked();
    }
}

bool ArchiveChunkMetaStore::UpdateArchiveState(const std::string& disk_id,
                                               const std::string& chunk_id,
                                               const std::string& archive_state,
                                               uint64_t version) {
    if (disk_id.empty() || chunk_id.empty() || archive_state.empty() || !inited_) {
        return false;
    }
    ...
    const ArchiveChunkMeta persisted = it->second;
    if (!AppendWalRecordLocked("U\t" + SerializeMetaLine(persisted))) {
        return false;
    }
    ++updates_since_snapshot_;
...
std::vector<ArchiveCandidateView> ArchiveChunkMetaStore::CollectCandidates(uint32_t max_candidates,
                                                                            uint64_t min_age_ms,
    ...
    for (const auto& item : metas_) {
        const ArchiveChunkMeta& meta = item.second;
        ...
        candidate.chunk_id = meta.chunk_id;
...
bool ArchiveChunkMetaStore::FlushSnapshot(std::string* error) {
    std::lock_guard<std::mutex> lock(mu_);
    return WriteSnapshotLocked(error);
}

std::string ArchiveChunkMetaStore::BuildKey(const std::string& disk_id, const std::string& chunk_id) {
    return disk_id + "|" + chunk_id;
}

bool ArchiveChunkMetaStore::ParseMetaLine(const std::string& line, ArchiveChunkMeta* meta) {
    if (!meta) {
        return false;
    }
    ...
    return !meta->disk_id.empty() && !meta->chunk_id.empty();
}

std::string ArchiveChunkMetaStore::SerializeMetaLine(const ArchiveChunkMeta& meta) {
...
std::vector<std::string> ArchiveChunkMetaStore::SplitTabs(const std::string& line) {
...
uint32_t ArchiveChunkMetaStore::Crc32(const std::string& data) {
...
bool ArchiveChunkMetaStore::WriteUint32LE(std::ostream* out, uint32_t value) {
    if (!out) {
        return false;
    }
    const char bytes[4] = {
...
bool ArchiveChunkMetaStore::ReadUint32LE(std::istream* in, uint32_t* value) {
    if (!in || !value) {
        return false;
    }
...
bool ArchiveChunkMetaStore::FsyncPath(const std::string& path) {
    if (path.empty()) {
        return false;
    }
#ifdef _WIN32
...
bool ArchiveChunkMetaStore::LoadSnapshotLocked(std::string* error) {
    ...
    while (std::getline(in, line)) {
        ...
        metas_[BuildKey(meta.disk_id, meta.chunk_id)] = std::move(meta);
    }
    return true;
}
...
bool ArchiveChunkMetaStore::ReplayWalLocked(std::string* error) {
    ...
    if (first == 'U' || first == 'D') {
        ...
        if (ec) {
            if (error) {
                *error = "failed to truncate legacy wal: " + wal_path_;
            }
            return false;
        }
        return true;
    ...
    while (true) {
        ...
        if (op == 'U') {
            ...
            metas_.erase(BuildKey(parts[0], parts[1]));
        } else {
            need_truncate = true;
            break;
        }
...
bool ArchiveChunkMetaStore::ReplayLegacyWalLocked(std::string* error) {
...
bool ArchiveChunkMetaStore::MaybeSnapshotLocked() {
...
bool ArchiveChunkMetaStore::WriteSnapshotLocked(std::string* error) {
...
void ArchiveChunkMetaStore::EvictIfNeededLocked() {
    while (metas_.size() > max_chunks_) {
        ...
        const ArchiveChunkMeta evicted = victim->second;
        metas_.erase(victim);
        if (wal_out_.is_open() &&
            AppendWalRecordLocked("D\t" + evicted.disk_id + "\t" + evicted.chunk_id)) {
            ++updates_since_snapshot_;
        }
    }
}
