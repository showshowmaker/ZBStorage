...
bool TestMdsNamespace(const std::string& endpoint, const std::string& uniq, std::string* detail) {
    ...
    uint64_t inode_id = 0;
    {
        ...
        req.set_gid(0);
        req.set_replica(1);
        req.set_chunk_size(4 * 1024 * 1024ULL);
...
bool TestChunkRw(const std::string& endpoint,
...

    {
        ...
        if (cntl.Failed()) {
            if (detail) {
                *detail = "WriteChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
        if (resp.status().code() != zb::rpc::STATUS_OK) {
            if (detail) {
                *detail = "WriteChunk status=" + std::to_string(resp.status().code()) + " msg=" + resp.status().message();
            }
            return false;
        }
    }

    {
        ...
        if (cntl.Failed()) {
            if (detail) {
                *detail = "ReadChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
        if (resp.status().code() != zb::rpc::STATUS_OK) {
            if (detail) {
                *detail = "ReadChunk status=" + std::to_string(resp.status().code()) + " msg=" + resp.status().message();
            }
            return false;
        }
        if (resp.data() != payload) {
            if (detail) {
                *detail = "ReadChunk payload mismatch";
            }
            return false;
        }
    }

    {
        ...
        if (cntl.Failed()) {
            if (detail) {
                *detail = "DeleteChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
        if (resp.status().code() != zb::rpc::STATUS_OK) {
            if (detail) {
                *detail = "DeleteChunk status=" + std::to_string(resp.status().code()) + " msg=" + resp.status().message();
            }
            return false;
        }
    }
    ...
    if (detail) {
        *detail = "disk=" + disk_id + " chunk rw OK";
    }
    return true;
}
...
bool TestOptical(const std::string& endpoint,
    ...
    const std::string chunk_id = "optical_chunk_" + uniq;
...

    {
        ...
        if (cntl.Failed()) {
            if (detail) {
                *detail = "optical plain WriteChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
    ...
    zb::rpc::WriteChunkReply write_resp;
    {
        ...
        req.set_file_chunk_index(0);
        brpc::Controller cntl;
        stub.WriteChunk(&cntl, &req, &write_resp, nullptr);
        if (cntl.Failed()) {
            if (detail) {
                *detail = "optical archive WriteChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
...

    {
        ...
        if (cntl.Failed()) {
            if (detail) {
                *detail = "optical ReadChunk rpc failed: " + cntl.ErrorText();
            }
            return false;
        }
        if (resp.status().code() != zb::rpc::STATUS_OK) {
            if (detail) {
                *detail = "optical ReadChunk status=" + std::to_string(resp.status().code()) + " msg=" + resp.status().message();
            }
            return false;
        }
        if (resp.data() != payload) {
            if (detail) {
                *detail = "optical ReadChunk payload mismatch";
            }
            return false;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    detail.clear();
    summary.Add("real_node.chunk_rw",
                TestChunkRw(FLAGS_real, FLAGS_real_disk, "real_smoke_chunk_" + uniq, "real_payload_" + uniq, &detail),
                detail);

    detail.clear();
    summary.Add("virtual_node.chunk_rw",
                TestChunkRw(FLAGS_virtual_node,
                            FLAGS_virtual_disk,
                            "virtual_smoke_chunk_" + uniq,
