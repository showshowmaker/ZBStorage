...
- **元数据服务（MDS）**：维护目录树、inode、layout 与 PG 相关元数据，元数据持久化到 RocksDB。
...
- 文件按 object unit 切分，MDS 返回 layout 后客户端直连数据节点做读写
- Real Node 算法化路径：`<mount>/<2hex>/<2hex>/<object_id>`
...
- `src/mds`：元数据服务（RocksDB 存储、inode/layout/PG 元数据管理）
...
- 光盘节点只接受归档写入（带 `archive_op_id`），普通 object 写入会被拒绝。
